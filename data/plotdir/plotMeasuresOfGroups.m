    ...
    for cIdx = 1:length(cons) %for both consomic strains, 12 and 14
        ...
        for measIdx = 1:length(meas) %for all measures
            ...
            for grpIdx = 1:length(grp) %for both groups, gene and protein
                ...
                %subplot(2,2,plotNr);
                hold on;
                plot(data(:,1), data(:,2), char(grpCol(grpIdx)));
                hold off;
            ...
            %plotNr = plotNr + 1;
            legend('SNPed Genes','d.e. Proteins');
            saveas(gcf,sprintf(outfmt, c, char(g), char(mea)), 'png');
            saveas(gcf,sprintf(outfmt, c, char(g), char(mea)), 'fig');
